r"""
PLKD (Projection Learning Knowledge Distillation)
"""
...
from .pre import prediect, predict_student
import os
...
def train_plkd(adata, gmt_path, project=None, pre_weights='', label_name='Celltype', max_g=300, max_gs=300, mask_ratio=0.015, n_unannotated=1, batch_size=8, embed_dim=48, depth=2, num_heads=4, lr=0.001, epochs=10, lrf=0.01, alpha=0.5, temperature=4.0, student_hidden=[256, 128], student_dropout=0.1):
    ...
    adata
        Single-cell datasets
    gmt_path
        The name or path of mask to be used.
    project
        The name of project.
    pre_weights
        The path to the pre-trained Teacher weights. If '', Teacher is trained from scratch.
    label_name
        The column name of the label.
    max_g, max_gs, mask_ratio, n_unannotated
        Mask generation parameters.
    batch_size
        Batch size.
    embed_dim, depth, num_heads
        Teacher Transformer parameters.
    lr
        Learning rate.
    epochs
        Number of epochs for both Teacher and Student training phases.
    lrf
        Cosine Annealing hyper-parameter.
    alpha
        Distillation loss weight (alpha * KD + (1-alpha) * CE).
    temperature
        Distillation temperature.
    student_hidden
        List of hidden layer dimensions for the Student MLP.
    student_dropout
        Dropout rate for the Student MLP.
    """
...
def pre_student(adata, model_weight_path, project, hidden_dims=[256, 128], dropout=0.1, batch_size=50, cutoff=0.1):
    r"""
    Predict query data with the PLKD Student model.
    ...
    adata
        Query single-cell datasets.
    model_weight_path
        The path to the pre-trained Student weights.
    project
        The name of project.
    hidden_dims
        List of hidden layer dimensions for the Student MLP. Must match training config.
    dropout
        Dropout rate.
    batch_size
        Batch size.
    cutoff
        Probability cutoff for 'Unknown' class.
        
    Returns
    -------
    adata
        adata.X : Latent representation
        adata.obs['Prediction'] : Predicted labels
        adata.obs['Probability'] : Probability of the prediction
    """
    return predict_student(adata, model_weight_path, project, hidden_dims=hidden_dims, dropout=dropout, batch_size=batch_size, cutoff=cutoff)
